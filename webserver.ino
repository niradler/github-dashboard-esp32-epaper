void setupWebServer() {
  ...
  server.on("/api/wifi/ap-password", HTTP_POST, handleChangeAPPassword);
...
void handleChangeAPPassword() {
  ...
  String password = doc["admin_password"].as<String>();
  String newAPPassword = doc["new_ap_password"].as<String>();
  ...
  if (newAPPassword.length() < 8) {
    Serial.println("[WEB] ✗ AP password too short (min 8 characters)");
    server.send(400, "application/json", "{\"error\":\"AP password must be at least 8 characters\"}");
    return;
  }
  
  Serial.println("[WEB] Updating AP password...");
  config.wifi_ap_password = newAPPassword;
  saveConfig();
  
  Serial.println("[WEB] ✓ AP password updated successfully");
  server.send(200, "application/json", "{\"success\":true,\"message\":\"AP password updated successfully\"}");
}
